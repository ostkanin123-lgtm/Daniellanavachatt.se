...
func update_appearance() -> void:
	...
	match type:
		...
		SpaceType.STAR:
			...
			if is_node_ready():
				var star_scene = load("res://super_star_super_mario_bros.glb")
				if star_scene:
					var star = star_scene.instantiate()
					star.scale = Vector3(0.1, 0.1, 0.1) # Scale down the GLB
					star.position.y = 1.0
					add_child(star)
					# Add a rotation script or logic
					var star_script = load("res://scripts/star.gd")
					if star_script:
						star.set_script(star_script)
				else:
					# Fallback Sphere
...
func on_landed(player: Node3D) -> void:
	# Juice: Squish the space
	var tween = create_tween()
	tween.tween_property(mesh_instance, "scale", Vector3(1.1, 0.5, 1.1), 0.1)
	tween.tween_property(mesh_instance, "scale", Vector3(1.0, 1.0, 1.0), 0.2).set_trans(Tween.TRANS_BOUNCE)

	match type:
		SpaceType.BLUE:
			player.coins += 3
			if player.has_method("_on_landed_effect"): player._on_landed_effect(type)
		SpaceType.RED:
