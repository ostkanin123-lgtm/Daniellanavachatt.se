...
var cam_follow_speed: float = 5.0
var cam_rot_speed: float = 4.0
var cam_offset: Vector3 = Vector3(0, 6, 8)

func _space_stand_offset(index_on_space: int) -> Vector3:
	# Small side offset so players on same space are both visible.
	var x = -0.8 if index_on_space == 0 else 0.8
	return Vector3(x, 0.0, 0.0)

func _resolve_player_overlap() -> void:
	var buckets := {}
	for p in players:
		if p and p.current_space:
			var key = str(p.current_space.get_instance_id())
	...
	for key in buckets.keys():
		...
		for i in range(count):
			var p = arr[i]
			var base_pos = p.current_space.global_position
			if count == 1:
				p.global_position = base_pos
			elif count == 2:
				p.global_position = base_pos + _space_stand_offset(i)
			else:
				# Fallback spread for 3+ players.
				var offset_x = (float(i) - float(count - 1) * 0.5) * 0.8
				p.global_position = base_pos + Vector3(offset_x, 0.0, 0.0)

func _get_board_center() -> Vector3:
	# Prefer board spaces if they exist.
	...
	# Fallback to player positions.
	if players.size() > 0:
		var sum_players := Vector3.ZERO
		for p in players:
			if p:
				sum_players += p.global_position
		return sum_players / float(players.size())

	# Fallback to island/ground center when board isn't built yet.
	var ground := get_node_or_null("Ground")
	if ground and ground is Node3D:
...
func setup_board() -> void:
	...
	var total_spaces = 32
	var radius = 25.0 # Tight circle for board game feel
	...
	for i in range(total_spaces):
		...
		space.position = Vector3(x, 0.1, z) # Slightly above ground
	...
	if start_space:
		...
		var start_players: Array = [m, l]
		for i in range(start_players.size()):
			var p = start_players[i]
			if p:
				p.current_space = start_space
				p.global_position = start_space.global_position + _space_stand_offset(i)
...
func add_decorations():
	...
	if rock_scene:
		for i in range(50): 
			...
			add_child(rock)
			if Engine.is_editor_hint(): rock.owner = get_tree().edited_scene_root
			var angle = randf() * TAU
			var dist = randf_range(180, 300)
			rock.position = Vector3(cos(angle) * dist, -5.0, sin(angle) * dist)
			var s = randf_range(10.0, 25.0)
	...
	for i in range(25):
		...
		cloud.name = "GeneratedCloud_" + str(i)
		var m = SphereMesh.new()
		m.radius = randf_range(15, 30)
		m.height = m.radius * 0.5
		cloud.mesh = m
		...
		add_child(cloud)
		if Engine.is_editor_hint(): cloud.owner = get_tree().edited_scene_root
		cloud.position = Vector3(randf_range(-500, 500), randf_range(100, 200), randf_range(-500, 500))
	...
	for i in range(150):
		...
		add_child(flower)
		if Engine.is_editor_hint(): flower.owner = get_tree().edited_scene_root
		var angle = randf() * TAU
		var dist = randf_range(40, 450)
		flower.position = Vector3(cos(angle) * dist, -0.2, sin(angle) * dist)
...
func _ready() -> void:
	...
	var m = find_child("Mario", true)
	var l = find_child("Luigi", true)
	if m: players.append(m)
	if l: players.append(l)
	...
	# Start view: show the island/board center instead of a random angle.
	var center = _get_board_center()
	cam_target_pos = center + Vector3(0, 22, 26)
	cam_target_look_at = center + Vector3(0, 1.5, 0)
	main_camera.global_position = cam_target_pos
	main_camera.look_at(cam_target_look_at)
	
	# Connect build buttons
	var b_space = find_child("BuildSpaceBtn", true)
	if b_space: b_space.pressed.connect(func(): builder.select_item(0)) # Blue
	
	var b_reset = find_child("BuildResetBtn", true)
	if b_reset: b_reset.pressed.connect(_reset_players_to_board)
	
	var b_rock = find_child("BuildRockBtn", true)
	if b_rock: b_rock.pressed.connect(func(): builder.select_item(5)) # Rock
	
	var b_tree = find_child("BuildTreeBtn", true)
	if b_tree: b_tree.pressed.connect(func(): builder.select_item(6)) # Tree
	
	var b_flower = Button.new()
	b_flower.text = "+ Blomma"
	b_flower.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	b_flower.pressed.connect(func(): builder.select_item(7))
	if build_ui: build_ui.add_child(b_flower)
	
	var b_cloud = Button.new()
	b_cloud.text = "+ Moln"
	b_cloud.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	b_cloud.pressed.connect(func(): builder.select_item(8))
	if build_ui: build_ui.add_child(b_cloud)
	
	var b_delete = find_child("BuildDeleteBtn", true)
	if b_delete: b_delete.pressed.connect(func(): builder.select_item(9)) # Delete
	
	var build_toggle_btn = find_child("BuildButton", true)
	if build_toggle_btn: build_toggle_btn.pressed.connect(toggle_build_mode)
...
func start_intro():
	...
	var center = _get_board_center()
	main_camera.global_position = center + Vector3(0, 40, 50)
	cam_target_pos = main_camera.global_position
	cam_target_look_at = center
	...
	cam_target_pos = player.global_position + Vector3(5, 10, 12) # Dramatic angle
...
func _on_roll_button_pressed() -> void:
	...
	cam_offset = Vector3(0, 3.5, 4.5) # Super close
	cam_follow_speed = 8.0 
	update_camera_focus()
	
	# Show dice above player
	dice_3d.global_position = player.global_position + Vector3(0, 8.0, 0)
	dice_3d.visible = true
	
	# Excitement jump!
	player.play_animation("Victory")
	player._show_bubble("HÄR KOMMER JAG!")
	await player.jump()
	...
	# Back out for movement
	cam_offset = Vector3(0, 5.0, 7.0) 
	cam_follow_speed = 6.0
	
	# Move player in one continuous walk sequence
...
func trigger_minigame():
	...
	var center = _get_board_center()
	cam_target_pos = center + Vector3(0, 28, 34)
	cam_target_look_at = center
...
func show_winner():
	...
	# Confetti effect!
	for i in range(100):
		var confetti = MeshInstance3D.new()
		var m = SphereMesh.new()
		m.radius = 0.2
		confetti.mesh = m
		var mat = StandardMaterial3D.new()
		...
		add_child(confetti)
		confetti.global_position = winner.global_position + Vector3(randf_range(-5, 5), randf_range(10, 20), randf_range(-5, 5))
		
		var t = create_tween()
		t.tween_property(confetti, "global_position:y", winner.global_position.y, randf_range(1.0, 2.0)).set_trans(Tween.TRANS_BOUNCE)
	
	# Focus camera on winner
	current_player_index = players.find(winner)
	cam_offset = Vector3(0, 4.0, 5.0)
...
func _reset_players_to_board() -> void:
	var board = get_node_or_null("Board")
	if not board or board.get_child_count() == 0:
		info_label.text = "Placera minst en ruta först!"
		return
	
	var first_space = board.get_child(0)
	for p in players:
		if p:
			p.current_space = first_space
			p.global_position = first_space.global_position
	
	_resolve_player_overlap()
	
	info_label.text = "Spelare placerade på start-rutan!"
	
	# Snap camera instantly to the new position
	cam_offset = Vector3(0, 6, 8) # Reset to default game offset
	update_camera_focus(true)
	
	# If we are in build mode, we might want to stay high up after a brief look
	if is_build_mode:
		# Small delay so the user sees where Mario is, then return to build view
		await get_tree().create_timer(1.0).timeout
		if is_build_mode:
			cam_target_pos = Vector3(0, 45, 45)
			cam_target_look_at = Vector3.ZERO
