...
func _process(_delta: float) -> void:
	...
	if ray_dir.y != 0:
		...
		if t > 0:
			...
			if preview_item:
				if "Cloud" in placement_items[current_item_index]:
					preview_item.global_position = world_pos + Vector3(0, 150, 0)
				else:
...
func _place_item(pos: Vector3) -> void:
	...
	match item_name:
		...
		"Flower":
			...
			flower.global_position = pos + Vector3(0, -0.2, 0)
		"Cloud":
			var cloud = MeshInstance3D.new()
			cloud.name = "PlacedCloud"
			var m = SphereMesh.new()
			m.radius = randf_range(15, 25)
			...
			cloud.global_position = pos + Vector3(0, 150, 0)
		"Delete":
			_delete_at(pos)
