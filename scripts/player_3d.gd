...
func _ready() -> void:
	...
	if not has_node("NameLabel"):
		...
		label.text = player_name
		label.modulate = player_color
		label.position = Vector3(0, 5.5, 0) # Higher up due to scale
	...
	if model_path != "":
		# Ensure the model is correctly set up
		var model = get_node_or_null("PlayerModel")
		
		if not model:
			# Remove any other potential models
		...
		if model:
			# Scale correctly for our board - Mario's model is inherently very small
			if "mario" in model_path.to_lower():
				model.scale = Vector3(100.0, 100.0, 100.0) 
				model.position = Vector3(-0.8, 0, 0) # Offset to side
			elif "luigi" in model_path.to_lower():
				model.scale = Vector3(12.5, 12.5, 12.5)
				model.position = Vector3(0.8, 0, 0) # Offset to side
			else:
...
func _process(delta: float) -> void:
	...
	match current_anim_state:
		...
		"Walk":
			...
			var walk_speed = 15.0 if is_luigi else 12.0
			model.position.y = abs(sin(anim_timer * walk_speed)) * (0.25 if is_luigi else 0.45)
			model.rotation.z = sin(anim_timer * walk_speed) * 0.15
			model.rotation.y = lerp_angle(model.rotation.y, model.rotation.y + sin(anim_timer * walk_speed) * 0.1, delta)
			_apply_character_procedural_anim("Walk")
		"Victory":
			# Spin and jump
			model.rotate_y(delta * 12.0)
			model.position.y = 2.0 + abs(sin(anim_timer * 6.0)) * 3.0
			# Grow a bit during victory!
			var target_scale = 120.0 if "mario" in player_name.to_lower() else 16.0
			model.scale = model.scale.lerp(Vector3(target_scale, target_scale, target_scale), delta * 2.0)
...
func _apply_character_procedural_anim(state: String) -> void:
	...
	match state:
		"Idle":
			...
			if is_luigi:
				# Luigi: Hands down, slightly relaxed
				skeleton.set_bone_pose_rotation(l_arm, Quaternion(Vector3(0, 0, 1), deg_to_rad(-80)))
				skeleton.set_bone_pose_rotation(r_arm, Quaternion(Vector3(0, 0, 1), deg_to_rad(80)))
				
				if head != -1:
					var head_look = sin(anim_timer * 0.7) * 0.2
					skeleton.set_bone_pose_rotation(head, Quaternion(Vector3(0, 1, 0), head_look))
			else:
				# Mario: HERO POSE (Matches the reference image)
				var breath = sin(anim_timer * 1.5) * 0.02
				# Rotate on Z axis to bring arms DOWN from T-pose
				# Adjusting values to get that exact "A-pose" from the image
				skeleton.set_bone_pose_rotation(l_arm, Quaternion(Vector3(0, 0, 1), deg_to_rad(-75 + breath)))
				skeleton.set_bone_pose_rotation(r_arm, Quaternion(Vector3(0, 0, 1), deg_to_rad(75 - breath)))
				
				# Wide stance for Mario
				if l_leg != -1: skeleton.set_bone_pose_rotation(l_leg, Quaternion(Vector3(0, 0, 1), deg_to_rad(-10)))
				if r_leg != -1: skeleton.set_bone_pose_rotation(r_leg, Quaternion(Vector3(0, 0, 1), deg_to_rad(10)))
				
				if head != -1:
					skeleton.set_bone_pose_rotation(head, Quaternion(Vector3(1, 0, 0), deg_to_rad(5)))
...
func display_coin_change(amount: int) -> void:
	var label = Label3D.new()
	label.text = ("+" if amount > 0 else "") + str(amount) + " Mynt"
	label.modulate = Color(1, 1, 0) if amount > 0 else Color(1, 0, 0)
	...
	tween.tween_property(label, "position:y", 8.0, 1.0)
	tween.parallel().tween_property(label, "modulate:a", 0.0, 1.0)
	tween.tween_callback(label.queue_free)

func _on_landed_effect(type: BoardSpace3D.SpaceType) -> void:
	match type:
		BoardSpace3D.SpaceType.BLUE:
			display_coin_change(3)
			_show_bubble("WOOHOO!")
		BoardSpace3D.SpaceType.RED:
			display_coin_change(-3)
			_show_bubble("MAMMA MIA!")
		BoardSpace3D.SpaceType.BOWSER:
			display_coin_change(-10)
			_show_bubble("DOH!")
		BoardSpace3D.SpaceType.STAR:
			_show_bubble("YEAH! STJÄRNA!")
			var label = Label3D.new()
			label.text = "STJÄRNA!"
			label.modulate = Color(1, 0.9, 0)
			...
			add_child(label)
			var tween = create_tween()
			tween.tween_property(label, "scale", Vector3(1.5, 1.5, 1.5), 0.5)
...
func _show_bubble(text: String) -> void:
	...
	add_child(label)
	var t = create_tween()
	t.tween_property(label, "position:y", 10.0, 0.8)
	t.parallel().tween_property(label, "scale", Vector3(1.2, 1.2, 1.2), 0.4)
