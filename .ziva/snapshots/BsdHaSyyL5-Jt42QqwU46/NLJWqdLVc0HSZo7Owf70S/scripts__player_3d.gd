	...
	match state:
		"Idle":
			...
			if is_luigi:
				...
				# Slow head movement
				if head != -1:
					var head_look = sin(anim_timer * 0.7) * 0.35
					skeleton.set_bone_pose_rotation(head, Quaternion(Vector3(0, 1, 0), head_look))
			else:
				# Mario: HERO POSE (Matches the image)
				# Arms down and out
				var breath = sin(anim_timer * 1.5) * 0.05 # Subtle breathing
				skeleton.set_bone_pose_rotation(l_arm, Quaternion(Vector3(1, 0, 0), deg_to_rad(75 + breath * 10)) * Quaternion(Vector3(0, 0, 1), deg_to_rad(20)))
				skeleton.set_bone_pose_rotation(r_arm, Quaternion(Vector3(1, 0, 0), deg_to_rad(-75 - breath * 10)) * Quaternion(Vector3(0, 0, 1), deg_to_rad(-20)))
				...
				# Head looking forward confidently
				if head != -1:
