	...
	match type:
		SpaceType.GREEN:
			...
			if get_index() == 0:
				...
				start_label.font_size = 600 # Bigger
				start_label.outline_size = 100
				start_label.modulate = Color(1, 1, 0) # Gold
				start_label.position.y = 2.0
		...
		SpaceType.BLUE:
			...
			mat.emission = Color(0, 0, 0.5) # Soft blue glow
			mat.emission_energy_multiplier = 0.5
			mat.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
		SpaceType.RED:
			mat.albedo_texture = load("res://SMPJ_Red_Space.webp")
			mat.emission_enabled = true
			mat.emission = Color(0.5, 0, 0) # Soft red glow
			mat.emission_energy_multiplier = 0.5
			mat.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
		SpaceType.STAR:
