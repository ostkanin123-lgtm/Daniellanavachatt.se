...
func _ready() -> void:
	...
	for i in range(spaces.size()):
		...
		if i < side_length:
			x = i * spacing
			z = 0
		elif i < side_length * 2:
			x = side_length * spacing
			z = (i - side_length) * spacing
		elif i < side_length * 3:
			x = (side_length * 3 - i) * spacing
			z = side_length * spacing
		else:
			x = 0
			z = (side_length * 4 - i) * spacing
			
		spaces[i].position = Vector3(x - (side_length * spacing / 2.0), 0, z - (side_length * spacing / 2.0))
		
		var next_idx = (i + 1) % spaces.size()
		spaces[i].next_space = spaces[next_idx]
		
		# Variety
		if i == 5: spaces[i].type = BoardSpace3D.SpaceType.STAR
		elif i == 15: spaces[i].type = BoardSpace3D.SpaceType.BOWSER
		elif i % 4 == 0: spaces[i].type = BoardSpace3D.SpaceType.RED
		elif i % 3 == 0: spaces[i].type = BoardSpace3D.SpaceType.GREEN
