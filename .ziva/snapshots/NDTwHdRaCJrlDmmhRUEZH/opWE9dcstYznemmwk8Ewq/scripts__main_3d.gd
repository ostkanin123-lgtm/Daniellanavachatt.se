...
func _ready() -> void:
	...
	if $Board.get_child_count() == 0:
		...
		for i in range(24): # A bit more spaces
			var space = space_scene.instantiate()
			$Board.add_child(space)
			
	# Link board spaces and position them in a nice layout (Oval/Circle)
	...
	for i in range(total_spaces):
		...
		var z = sin(angle) * radius * 1.5 # Oval
		
		# Add some noise/waviness to the path
		x += sin(i * 0.5) * 2.0
		z += cos(i * 0.5) * 2.0
