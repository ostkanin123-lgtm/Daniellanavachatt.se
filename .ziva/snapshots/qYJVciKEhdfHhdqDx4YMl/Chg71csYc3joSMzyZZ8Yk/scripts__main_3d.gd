...
func _ready() -> void:
	...
	if has_node("Ground"):
		$Ground.scale = Vector3(1.5, 1, 1.5) # Make it even bigger
	...
	for i in range(players.size()):
		...
		var offset = Vector3(-0.4 if i == 0 else 0.4, 0.5, 0)
	...
	# start_intro() # Moved to input check
...
func start_intro():
	...
	# Initial wide view
	main_camera.global_position = Vector3(0, 80, 100) # Higher up
	cam_target_pos = main_camera.global_position
	cam_target_look_at = Vector3.ZERO
	
	info_label.text = "GÖR ER REDO!"
	info_label.scale = Vector2(1, 1) # Label3D and Label handles scale differently
	...
	cam_target_pos = player.global_position + Vector3(5, 12, 15) # Dramatic angle
	cam_target_look_at = player.global_position
	
	intro_tween.tween_property(main_camera, "global_position", cam_target_pos, 2.5).set_trans(Tween.TRANS_SINE)
	...
	pulse_tween.tween_property(info_label, "scale", Vector2(1.5, 1.5), 0.2)
	pulse_tween.tween_property(info_label, "scale", Vector2(1.0, 1.0), 0.1)
