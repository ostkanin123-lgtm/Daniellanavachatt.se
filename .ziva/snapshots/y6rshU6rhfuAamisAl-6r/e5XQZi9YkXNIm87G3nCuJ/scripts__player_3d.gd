...
func _process(delta: float) -> void:
	...
	match current_anim_state:
		"Idle":
			...
			# Luigi specific: slightly nervous shaking
			if "luigi" in player_name.to_lower():
				model.rotation.z = sin(anim_timer * 15.0) * 0.02
			_apply_character_procedural_anim("Idle")
		"Walk":
			# More intense bobbing/waddle
			model.position.y = abs(sin(anim_timer * 10.0)) * 0.3
			model.rotation.z = sin(anim_timer * 10.0) * 0.1
			if "luigi" in player_name.to_lower():
				# Luigi's signature bouncy walk
				model.rotation.x = sin(anim_timer * 15.0) * 0.05
