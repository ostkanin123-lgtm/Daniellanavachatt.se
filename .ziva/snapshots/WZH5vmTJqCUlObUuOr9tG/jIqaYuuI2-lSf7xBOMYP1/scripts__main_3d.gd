...
func _set_refresh_board(val: bool) -> void:
	if val:
		# Need to populate players manually if in editor
		players = []
...
func setup_board() -> void:
	...
	for i in range(total_spaces):
		...
		if spaces[i].has_method("update_appearance"):
...
func _ready() -> void:
	...
	if Engine.is_editor_hint():
		# In editor, if board is empty, set it up automatically
		if has_node("Board") and $Board.get_child_count() == 0:
			setup_board()
		return
