...
func setup_board() -> void:
	...
	# Clear existing first to avoid doubles
	for child in $Board.get_children():
		child.queue_free()
	
	# Wait a frame for queue_free if in editor
	...
	if has_node("Ground"):
		$Ground.scale = Vector3(200, 1, 200)

	# Setup initial player positions in editor
	var start_space = spaces[0] if spaces.size() > 0 else null
	if start_space:
		# Temporarily find players for positioning
		var editor_players = []
		var m = find_child("Mario", true)
		var l = find_child("Luigi", true)
		if m: editor_players.append(m)
		if l: editor_players.append(l)
		
		for i in range(editor_players.size()):
			var p = editor_players[i]
