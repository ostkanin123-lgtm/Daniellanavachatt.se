...
@onready var info_label: Label = find_child("DiceLabel", true) if find_child("DiceLabel", true) else find_child("InfoLabel", true)
@onready var coin_label: Label = find_child("Player1Info", true) if find_child("Player1Info", true) else find_child("CoinLabel", true)
...
@onready var builder: Builder = find_child("Builder", true)
@onready var build_ui: Control = find_child("BuildUI", true)

func _ready() -> void:
	# Connect buttons
	if dice_button:
		dice_button.pressed.connect(_on_roll_button_pressed)
	
	var build_btn = find_child("BuildButton", true)
	if build_btn:
		build_btn.pressed.connect(toggle_build_mode)
		
	# Connect build menu buttons
	var b_space = find_child("BuildSpaceBtn", true)
	if b_space: b_space.pressed.connect(func(): builder.select_item(0))
	var b_rock = find_child("BuildRockBtn", true)
	if b_rock: b_rock.pressed.connect(func(): builder.select_item(1))
	var b_tree = find_child("BuildTreeBtn", true)
	if b_tree: b_tree.pressed.connect(func(): builder.select_item(2))
	var b_delete = find_child("BuildDeleteBtn", true)
	if b_delete: b_delete.pressed.connect(func(): builder.select_item(3))
	...
	var radius = 30.0 # Even larger as requested
	...
	for i in range(total_spaces):
		...
		elif (i % 6 == 0): # More occasional red
	...
	# Ground is handled in the scene now
	if has_node("Ground"):
		$Ground.scale = Vector3(100, 1, 100) # Gigantisk mark för att täcka allt

	# Set players
	players = []
	var mario = find_child("Mario", true)
	var luigi = find_child("Luigi", true)
	...
	if start_space:
		for i in range(players.size()):
			...
			# Offset players so they don't overlap at start
			var offset = Vector3(-0.6 if i == 0 else 0.6, 0.5, 0)
			p.global_position = start_space.global_position + offset
			
			# Make players look towards the next space
			if start_space.next_space:
				p.look_at(start_space.next_space.global_position, Vector3.UP)
				p.rotation.x = 0
	...
	info_label.text = "TRYCK PÅ ENTER FÖR ATT STARTA!"
	update_ui()
...
func add_decorations():
	...
	if rock_scene:
		...
		for i in range(40): # Fler berg för en tätare ring
			...
			# Banan är på radie 30, så vi lägger bergen på 70-120
			var angle = randf() * TAU
			var dist = randf_range(75, 130) 
