...
func add_decorations():
	...
	if rock_scene:
		# Utvändiga stenar (Bakgrund)
		for i in range(25):
			var rock = rock_scene.instantiate()
			add_child(rock)
			var angle = randf() * TAU
			var dist = randf_range(55, 90) # Långt utanför banan (som är på 30)
			rock.position = Vector3(cos(angle) * dist, -1.0, sin(angle) * dist)
			rock.scale = Vector3.ONE * randf_range(1.5, 4.0) # Större berg i bakgrunden
			rock.rotate_y(randf() * TAU)
			
		# Invändiga stenar (I mitten av cirkeln)
		for i in range(8):
			var rock = rock_scene.instantiate()
			add_child(rock)
			var angle = randf() * TAU
			var dist = randf_range(0, 15) # Inuti cirkeln, långt från banan
			rock.position = Vector3(cos(angle) * dist, -0.5, sin(angle) * dist)
